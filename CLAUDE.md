...
- **OG image is static** — `public/og.png`, regenerated by `node scripts/generate-og.mjs` (headless Chromium screenshot of the brand card). Dynamic `next/og` routes were removed: they timed out on Vercel's Edge runtime in prod (see decision #21). Don't re-add a dynamic OG route without solving that first. Keep the file ≤ ~500 KB so WhatsApp doesn't reject it.
- **React Hook Form `watch()`** triggers a React Compiler warning. It's a known incompatibility, accepted.
- **lucide-react (this version) dropped brand icons** — `Github` and friends don't exist. Inline an SVG for brand marks (see `src/components/shared/site-footer.tsx`). Don't waste a round-trip importing them.
- **One-off / throwaway scripts go in `/tmp/`, not the repo** — or prefix them `scripts/_*` (gitignored). Sandbox blocks `rm`/`rm -rf`, so anything you drop in the repo to "clean up later" tends to get stuck as an untracked file.
